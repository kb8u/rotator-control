...
use lib '/home/rjd/rotator-control/lib';
...
my $CONFIG_DIR = $opt_d ? $opt_d : File::HomeDir->my_data().'/rotator-control';
...
sub repeat_handler {
  my ($canv,$line_ref,
      $direction_entry_ref,$text_direction_ref,$text_speed_ref)= @_;

  my $result;
  my ($x,$y,$radians);
  return unless query_rotor("\r",\$result,19);
  ...
  unless ($speed =~ /^\d$/ && $speed > 0 && $speed < 10) { return }
...
sub tkhelp {
  my @helparray = ([{-title => 'rotator-control',
                     ...
                     -text => "Computer control a M\N{SUPERSCRIPT TWO} RC2800PX antenna rotor control"}],
                   [{-title => 'Application basic usage',
                     -header => "Application basic usage",
                     -text => 
'You may need to set the serial port in the Setup menu first (see next section).  If everything is working you should see numbers in the Heading and Speed text boxes and a green line on the map will show where the rotator is pointing.
...
my $stop_Button = $info_frame->Button
  (
   -text => 'STOP',
   -command => [ sub { $sp->write("s\r") if $sp } ],
  );
...
=head1 rotator-control help

=head2 usage

You may need to set the serial port in the Setup menu first (see below).  If
everything is working you should see numbers in the Heading and Speed text boxes
